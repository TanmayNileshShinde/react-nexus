...
const SnakeGame = () => {
  ...
  const MenuCard = ({ icon: Icon, title, subtitle, onClick, color }) => (
    <div onClick={onClick} className="glass-panel" style={{ 
      ...
      transition: 'all 0.3s ease', boxShadow: '0 4px 30px rgba(0, 0, 0, 0.1)', marginBottom: '15px'
    }}
      ...
      {view === 'game' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#00af3a', textShadow: '0 0 10px rgba(0, 175, 58, 0.5)' }}>
              SCORE: {score}
            </div>
            
            {!user ? (
              <button onClick={handleLogin} style={{ background: 'white', color: '#0f172a', border: 'none', padding: '6px 12px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 'bold', cursor: 'pointer' }}>Login to Save</button>
            ) : (
              <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div>
                  <div style={{ fontSize: '0.8rem', color: 'white', fontWeight: 'bold' }}>{user.displayName}</div>
                  <div style={{ fontSize: '0.7rem', color: '#ffd700' }}>Best: {stats.snakeHighScore}</div>
                </div>
                <img src={user.photoURL} alt="User" referrerPolicy="no-referrer" style={{ width: 30, height: 30, borderRadius: '50%', border: '2px solid #00af3a' }} />
              </div>
            )}
          </div>
              ...
              leaderboardData.map((player, index) => (
                <div key={index} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px', marginBottom: '10px', borderRadius: '12px', background: index === 0 ? 'linear-gradient(90deg, rgba(255,215,0,0.2) 0%, rgba(255,255,255,0.05) 100%)' : 'rgba(255,255,255,0.05)', border: index === 0 ? '1px solid rgba(255,215,0,0.5)' : '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#00af3a', fontWeight: 'bold', fontSize: '1.2rem' }}>{player.snakeHighScore || 0}</div>
                    <div style={{ fontSize: '0.7rem', opacity: 0.5, color: 'white' }}>High Score</div>
                  </div>
                </div>
              ))
            )}
