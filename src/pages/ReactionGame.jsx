...
const ReactionGame = () => {
  ...
  const Light = ({ active }) => (
    <div style={{
      ...
      boxShadow: active 
        ? '0 0 25px #ff0055, 0 0 50px #ff0055, inset 0 0 10px #ff88aa' 
  ...
  return (
    <div className="glass-panel" style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '25px' }}>
        {!user ? (
          <button onClick={handleLogin} style={{ 
            ...
            borderRadius: '25px', fontWeight: '900', cursor: 'pointer', boxShadow: '0 0 20px rgba(255,255,255,0.4)'
          }}>LOGIN</button>
        ) : (
          <div style={{ 
             ...
             <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '0.75rem', color: '#00f3ff', fontWeight: '800' }}>BEST: {bestTime || '---'}</div>
             </div>
      ...
      <div onClick={handleReaction} style={{ 
        ...
        {view === 'game' ? (
          <>
            ...
            {gameState === 'ready' && <h1 style={{ color: '#00ff88', fontSize: '6rem', fontWeight: '950', textShadow: '0 0 40px #00ff88' }}>GO!</h1>}
            {gameState === 'foul' && <h2 style={{ color: '#ff4444', textShadow: '0 0 20px #ff4444' }}>JUMP START!</h2>}
            {gameState === 'result' && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.2rem', opacity: 0.5 }}>TIME</div>
                <div style={{ fontSize: '6rem', fontWeight: '1000', color: resultTime < 300 ? '#00ff88' : 'white', textShadow: resultTime < 300 ? '0 0 50px rgba(0,255,136,0.5)' : 'none' }}>{resultTime}ms</div>
              </div>
            )}
          </>
        ) : (
          <div style={{ width: '100%', padding: '20px', height: '100%', overflowY: 'auto' }}>
            <h3 style={{ color: '#ffcc00', textAlign: 'center', letterSpacing: '3px' }}>HALL OF FAME</h3>
            {leaderboard.map((u, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', background: 'rgba(255,255,255,0.04)', marginBottom: '8px', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <span><span style={{ color: '#ffcc00' }}>#{i+1}</span> {u.displayName}</span>
      ...
      <div style={{ display: 'flex', gap: '12px', marginTop: '30px' }}>
        ...
        <button className={styles.button} onClick={fetchLeaderboard} style={{ flex: 1, background: 'rgba(188, 19, 254, 0.1)', border: '1px solid rgba(188, 19, 254, 0.5)' }}>
            {view === 'game' ? <><Trophy size={20}/></> : <><ArrowLeft size={20}/></>}
        </button>

        <button className={styles.button} onClick={startSequence} style={{ flex: 2, background: 'rgba(0, 243, 255, 0.15)', border: '1px solid #00f3ff', color: '#00f3ff', fontWeight: '900' }}>
