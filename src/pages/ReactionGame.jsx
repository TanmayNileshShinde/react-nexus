...
const ReactionGame = () => {
  ...
  const Light = ({ active }) => (
    <div style={{
      ...
      // Multi-layer shadow for intense glow
      boxShadow: active 
  ...
  return (
    <div className="glass-panel" style={{ 
        ...
        // Matches Page 1 border style
        border: '1px solid rgba(0, 243, 255, 0.3)',
        boxShadow: '0 0 40px rgba(0, 243, 255, 0.1)'
    }}>
      
      {/* HEADER: Matches Page 1 Title Gradient */}
      <h2 style={{ 
          textAlign: 'center', 
          fontSize: '2.5rem', 
          fontWeight: '900',
      ...
      {/* USER PROFILE: Matches Page 3 Pill Style */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '25px' }}>
        {!user ? (
          ...
          <div style={{ 
              background: 'rgba(255, 255, 255, 0.07)', padding: '5px 15px', borderRadius: '30px', 
             ...
             <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '0.7rem', color: '#00f3ff', fontWeight: '800' }}>BEST: {bestTime ? `${bestTime}ms` : '---'}</div>
             </div>
      ...
      {/* GAME CONSOLE: Darker background for contrast */}
          ...
          <>
            {gameState === 'idle' && (
              <div style={{ textAlign: 'center' }}>
                ...
                <h3 style={{ color: '#00f3ff', letterSpacing: '4px', marginTop: '15px', opacity: 0.8 }}>SYSTEM READY</h3>
              </div>
            )}
            ...
            {gameState === 'result' && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.2rem', opacity: 0.6, letterSpacing: '2px' }}>REACTION TIME</div>
                <div style={{ 
                    fontSize: '6rem', fontWeight: '900', color: resultTime < 300 ? '#00ff88' : 'white',
                    textShadow: resultTime < 300 ? '0 0 50px rgba(0,255,136,0.6)' : 'none'
                }}>{resultTime}<span style={{ fontSize: '1.5rem' }}>ms</span></div>
              </div>
            )}
          </>
        ) : (
          ...
          <div style={{ width: '100%', padding: '20px', height: '100%', overflowY: 'auto' }}>
            ...
            {leaderboard.map((u, i) => (
              <div key={i} style={{ 
                  ...
                  background: 'rgba(255,255,255,0.05)', marginBottom: '8px', borderRadius: '12px',
                  border: '1px solid rgba(255,255,255,0.05)'
              }}>
                <span style={{ fontWeight: 'bold' }}><span style={{ color: '#ffcc00' }}>#{i+1}</span> {u.displayName}</span>
                <span style={{ color: '#00f3ff', fontWeight: 'bold' }}>{u.best_reaction}ms</span>
              </div>
            ))}
          </div>
        )}
      ...
      {/* NAVIGATION: Matches Page 3 Layout */}
